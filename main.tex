...
\right].\nonumber 
%\\
%L &=  (\bar{\Delta}+\alpha|\dot{p}^r| + \bar{\text{s}}_\Phi\bar{T})\left[
%\begin{array}{cc}
%1&1
%\end{array}
%\right]. \label{eq:QL}
...
%Since the gain matrices $K_p$ and $K_v$ are designed to make the system matrix $A$ Hurwitz, we can find the positive definite matrix $P$ satisfying $PA + A^\top P < 0$. Accordingly, it is possible to define $Q = -\frac{1}{2}(PA +A^\top P) \geq 0$. 
...
%Then, with the Lyapunov candidate function $V = \frac{1}{2}e^\top P e$, the stability analysis on eq. \eqref{eq:translationalError} can be carried out as follows: 
%\begin{align}
%\dot{V} &= \textstyle{\frac{1}{2}}e^\top (PA + A^\top P)e + e^\top P B (\Delta + \text{s}_\Phi|\ddot{x}^d|\bar{u}) \nonumber \\
%&= -e^\top Q e \label{eq:lyapForFunnel} \\
%&\;\;\;\; + e^\top P B (\Delta + \text{s}_\Phi |-K_p e_p - K_v e_v +ge_3+{\ddot{x}}^r|\bar{u}) \nonumber  \\
...
%Then, 
%\begin{equation}
%\dot{V} \leq (-\nu+\bar{\text{s}}_\Phi\eta_p\eta_k)\theta|e|^2,\;\;\; \forall |e| \geq L \left[\begin{array}{ll}\bar{\Delta} & \bar{T}\end{array}\right]^\top \label{eq:forConstraint} 
%\end{equation}
%where
%\begin{equation}
...
%\\ 
%&= -e^\top Q_\mathcal{F} e + e^\top \dot{P}_\mathcal{F}e \label{eq:vdot1} \\
