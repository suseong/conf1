...
Here, $\psi$ represents the rotation of a multirotor about $z_b$ axis.
...
\right]. 
\end{equation}
%When the input terms are zeros, the error dynamic system becomes globally exponentially stable system. Also, the above system is globally Lipschitz in $e$, $\Delta$, and $\Phi$. Therefore, by 
...
\end{equation}
where 
$\nu = \lambda_{\min}(Q)$, $\eta_p = \lambda_{\max}(P)$, and $\eta_k = \lambda_{\max}(\text{blockdiag}\{K_p,K_d\})$.
%\begin{equation}
%\begin{array}{ll}
%\nu = \lambda_{\min}(Q),&\eta_p = \lambda_{\max}(P) \\
%\eta_k = \lambda_{\max}(\text{blockdiag}\{K_p,K_d\}). &
%\end{array}
%\end{equation}
...
&= -e^\intercal Q e + e^\intercal P B (\Delta + \text{s}_\Phi |-K e+ge_3+{\ddot{x}}^r|\bar{u}) \label{eq:lyapForFunnel} \\
...
Also, the term $L$ could be considered as the function of the error $\text{s}_\Phi$. By increasing the attitude error, the norm of $L$ would get bigger. It means that the error $|e|$ will be bounded with larger radius. It implies that the attitude control should be achieved as accurate as possible. 

Heretofore, the stability of the translational motion of a multirotor has been analyzed. The error $|e|$ will be bounded, and the bound is determined by the norm of disturbance, norm of the nominal thrust, and the attitude control quality. However, even though we know how the error would behave, we cannot find the actual bound of the error state. It could be found by computing funnels explained in the next section.

\section{Generating funnel libraries}
