...
%the found ultimate bounds are similar regardless of initial condition if the disturbance $\Delta_k$ is set to similar value. 
...
% with respect to the maximum disturbance during each time segment. For example, in the figure, $\Delta(t_{n-2}) = \Delta_l$, $\Delta(t_{n-1}) = \Delta_{l}$, $\Delta_M(t_{n}) = \Delta_l$, $\Delta_M(t_{n+1}) = \Delta_{l+2}$, and $\Delta_M(t_{n+2}) = \Delta_{l+2}$.} 
...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
	...
	\State $i_0 \gets i$ (\text{index of }$\bar{F}$)
	\State $j_0 \gets j$ (\text{initial condition of }$\bar{F}$)
      \For{$n \gets 1 \textrm{ to } N$}
				...
				\State \text{where} $\Delta_M = \max(\Delta(t)),\;\forall t\in(t_{n-1},t_n]$
				\State $\Delta_{n} \gets \Delta_l$
				\If{$\Delta_n = \Delta_{n-1}$}
				...
				\EndIf
      \EndFor
      \State \Return{$l_n$, $j_n$, $i_n$ for $n=\{1,\cdots,N\}$}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
%Let $\Delta_n$ denote the chosen disturbance level at $t_n$.
...
%By iterating the first and second procedures from the $t_0$, we can construct the funnel for all time interal $t_n$ $n=\{0,\cdots,N\}$. 
The overall procedure explained is summarized in the algorithm 1.

%As a remedy, we can mix and match multiple funnels we computed to generate a realistic funnel rather than conservative. 
%To take advantages from the funnels computed with discrete time and disturbances $\Delta_l$, we 
%assign one discretized disturbance among $\Delta_l$s  to represent the norm of disturbance $|\Delta(t)|$ for each time segment $t\in(t_{n-1},t_n]$. 
%Each funnel $F_{\Delta_{l}}^j$ is evaluated with the setting $|\Delta| \leq \Delta_l$. 
%In accordance with that, 
%Among all $l$ values, the obvious choice would be using the minimum one satisfying $|\Delta(t)| \leq \Delta_l$ during the time segment $t\in(t_{n-1},t_n]$. It is because that $\Delta_l$ is the closest to the actual $|\Delta(t)|$ as well as the condition required in eq. 16\eqref{eq:} is satisfied.
%If the step size between $\Delta_l$ and time intervals are fine enough, the actual disturbance could be well reflected in the problem set in the discretized manner. Let $\Delta_M$ denote the chosen disturbance level at each time interval.
%%And since $|\Delta(t)|\leq \Delta_l$, we can fulfill the condition required in eq. 16\eqref{eq:}.
%It is described in the first row of the fig. \ref{fig:combiningConcept}.
%In the same manner, once we have the history of $\Delta(t)$, we can compute the maximum disturbance $\Delta_M$ for each time segment $t\in(t_{n-1},t_n]$ as described in fig. \ref{fig:combiningConcept}. Furthermore, we can find the disturbance level $\Delta_l$ satisfying $\Delta_{l-1} < \Delta_M \leq \Delta_l$. 
%Then, the magnitude of disturbance during the time segment is close and smaller than $\Delta_l$. Thus, 
%the funnel $F_{\Delta_l}$ computed with the maximum disturbance $\Delta_l$ is expected to behave similar to the actual funnel. 
