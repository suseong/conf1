...
%     plot(sum(T(1:k))+T_,jerk_z(k,:))    
    ...
    subplot(4,1,3)
    hold on; alpha = 0.32; sinPhi = 0.0523; Delta = 0.5;
...
%%
% alpha = 0.32; sinPhi = 0.0523; Delta = 0.5;
...
curRegion = diag(1./(4*[0.05 0.05 0.05 0.1 0.1 0.1]).^2);
funnelIdx = [];
cnt = 1;
for k=1:length(rs_d.Time)
    ...
    for kk=1:length(funnel_b)-1
       if d < 0.1*kk+0.5 && d > 0.1*(kk-1)+0.5
           d = 0.1*kk+0.5;
           kk_ = kk;
           break;
       end
